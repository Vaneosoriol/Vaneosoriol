...
def execute_marvel_pipeline():
    ...
    def convert_list_to_string(item):
        if isinstance(item, list):
            return ', '.join(str(x) for x in item)
        return item

    # Aplicar la función a todas las celdas del DataFrame
    df = df.applymap(convert_list_to_string)
...
def send_alert_email():
    # Obtener el valor de "Series_Available" desde Redshift
    redshift_conn = BaseHook.get_connection("redshift_connection")
    conn = psycopg2.connect(
        host=redshift_conn.host,
        port=redshift_conn.port,
    ...
    cur = conn.cursor()
    cur.execute("SELECT SUM (series_available) FROM vane_carop19_coderhouse.info_marvel")
    total_series_available = cur.fetchone()[0]  # Obtener el valor de "Series_Available" desde Redshift

    if total_series_available < 70:
        ...
        email_task = EmailOperator(
            ...
            subject='Alerta de Airflow Marvel',
            html_content="<p>¡Alerta! Menos de 70 series disponibles.</p>",
            dag=dag,
        )

        email_task.execute()

# Definir la tarea que enviará la alerta
